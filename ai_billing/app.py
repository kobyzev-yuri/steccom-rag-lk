...
def render_model_management():
    ...
    default_rag_provider_index = 1 if current_rag_model == 'gpt-4o' else 0
    
    rag_provider = st.radio(
        "Выберите провайдера для RAG Assistant:",
        ["Ollama (локальные модели)", "GPT-4o (быстро и качественно)"],
        index=default_rag_provider_index,
        key="rag_provider_select"
    )
    
    if rag_provider == "Ollama (локальные модели)":
        import subprocess
        try:
            result = subprocess.run(['ollama', 'list'], capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                ...
                if available_models:
                    # Filter to only show current model if it's Ollama
                    if current_rag_model in available_models:
                        default_index = available_models.index(current_rag_model)
                    else:
                        default_index = 0
                    
                    selected_rag_model = st.selectbox(
                        "Модель Ollama для RAG Assistant:",
                        available_models,
                        index=default_index,
                        key="rag_ollama_model_select"
                    )
                    
                    if st.button("Применить Ollama модель для RAG", key="apply_rag_ollama_model"):
        ...
        st.info("🚀 GPT-4o - быстрая и качественная работа с базами знаний")
        st.markdown("""
        **Преимущества GPT-4o для RAG:**
        - ⚡ Быстрые ответы на вопросы по БЗ
        - 🎯 Высокое качество понимания контекста
        - 📊 Обработка сложных запросов
        - 🔍 Точный поиск по документам
        """)
        
        if st.button("Переключить RAG на GPT-4o", key="apply_rag_gpt4o"):
            st.session_state.rag_assistant_model = 'gpt-4o'
            # Update RAG system with GPT-4o
            if st.session_state.get('multi_rag'):
                try:
                    import os
    ...
    with col2:
        ...
        current_rag_model = st.session_state.get('rag_assistant_model', 'qwen2.5:1.5b')
        if current_rag_model == 'gpt-4o':
            st.success(f"🚀 GPT-4o (быстро)")
        else:
            st.info(f"🦙 Ollama: {current_rag_model}")
